...
const SPOTIFY_EN_AG_ID = 196005255745;

console.log("=== SPOTIFY EN ad group settings ===");
const settings = await customer.query(`
  SELECT ad_group.id, ad_group.name, ad_group.cpc_bid_micros, ad_group.type
  FROM ad_group
  WHERE ad_group.id = ${SPOTIFY_EN_AG_ID}
`);
...
const ads = await customer.query(`
  ...
  FROM ad_group_ad
  WHERE ad_group.id = ${SPOTIFY_EN_AG_ID} AND ad_group_ad.status != 'REMOVED'
`);
for (const a of ads) {
...
console.log("\n=== DB Spotify prices (sp_streams, source of truth) ===");
const sql = neon(env.DATABASE_URL);
const rows = await sql`
  SELECT qty, price::float AS price, popular
  FROM pricing
  WHERE service = 'sp_streams' AND active = true
  ORDER BY qty
`;
for (const r of rows) {
