...
const FB_EN_AG_ID = 201841124572;

console.log("=== FACEBOOK EN ad group settings ===");
const settings = await customer.query(`
  SELECT ad_group.id, ad_group.name, ad_group.cpc_bid_micros, ad_group.type
  FROM ad_group WHERE ad_group.id = ${FB_EN_AG_ID}
`);
...
const ads = await customer.query(`
  ...
  FROM ad_group_ad
  WHERE ad_group.id = ${FB_EN_AG_ID} AND ad_group_ad.status != 'REMOVED'
`);
for (const a of ads) {
  ...
  for (const h of ad.responsive_search_ad?.headlines || []) {
    console.log(`    "${h.text}"${h.pinned_field ? `  [PIN ${h.pinned_field}]` : ""}  (${h.text.length} chars)`);
  }
  console.log("  Descriptions:");
  for (const d of ad.responsive_search_ad?.descriptions || []) {
    console.log(`    "${d.text}"${d.pinned_field ? `  [PIN ${d.pinned_field}]` : ""}  (${d.text.length} chars)`);
  }
}

console.log("\n=== DB Facebook prices (fb_likes, source of truth) ===");
const sql = neon(env.DATABASE_URL);
const rows = await sql`
  SELECT qty, price::float AS price, popular
  FROM pricing
  WHERE service = 'fb_likes' AND active = true
  ORDER BY qty
`;
for (const r of rows) {
