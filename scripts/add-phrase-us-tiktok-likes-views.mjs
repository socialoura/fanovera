/**
 * Adds PHRASE keywords for TikTok likes & views on [US] Fanovera TikTok AG.
 *
 * Usage:
 *   node scripts/add-phrase-us-tiktok-likes-views.mjs          # dry-run
 *   node scripts/add-phrase-us-tiktok-likes-views.mjs --live   # execute
 */
...
async function getNegatives() {
  ...
  const campNeg = await customer.query(`SELECT campaign_criterion.keyword.text, campaign_criterion.keyword.match_type FROM campaign_criterion WHERE campaign.id=${US_CAMPAIGN_ID} AND campaign_criterion.type='KEYWORD' AND campaign_criterion.negative=true`);
  ...
  const sets = await customer.query(`SELECT shared_set.id FROM campaign_shared_set WHERE campaign.id=${US_CAMPAIGN_ID} AND campaign_shared_set.status!='REMOVED'`);
  for (const s of sets) {
...
console.log(`\nDone. Monitor search terms in ~2 weeks: node scripts/analyze-ag-search-terms.mjs ${AG_ID} 14 en`);
