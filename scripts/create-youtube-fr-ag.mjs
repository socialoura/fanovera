/**
 * Creates the YouTube FR ad group on campaign [FR] Fanovera with all assets.
 * Mirrors EN YouTube structure (views + subscribers in same AG).
 */
...
const POSITIVE_KEYWORDS = [
  ...
  "acheter 10000 vues youtube",
  "vues youtube pas cher",
  "payer vues youtube",
  "acheter abonnés youtube",
  "acheter abonné youtube",
  "achat abonnés youtube",
  "achat abonné youtube",
  "acheter subscribers youtube",
  "acheter 100 abonnés youtube",
  "abonnés youtube pas cher",
  "promouvoir vidéo youtube",
  "booster chaine youtube",
  "meilleur site vues youtube",
];

const YOUTUBE_SPECIFIC_NEGATIVES = [
  "youtube ads", "youtube premium", "youtube music", "youtube kids",
  "youtube tv", "youtube shorts", "youtube vanced", "youtube app",
  "youtube login", "créer compte youtube", "supprimer youtube",
  "youtube downloader", "youtube partner program", "youtube go",
  "youtube originals",
];
...
const RSA = {
  ...
  descriptions: [
    { text: "Développez votre présence sur YouTube, Instagram, Spotify et Tiktok. Code -5% : FANO5." },
...
const PRICE_OFFERINGS = [
  { header: "Pack de 1000", description: "Le Moins Cher du Marché", amount_eur: 4.99 },
  { header: "Pack de 5000", description: "Le Moins Cher du Marché", amount_eur: 18.99 },
  { header: "Pack de 10000", description: "Le Moins Cher du Marché", amount_eur: 34.99 },
];
...
const ALL_NEGATIVES = [
  ...igNegatives,
  ...YOUTUBE_SPECIFIC_NEGATIVES.map((text) => ({ text, match_type: enums.KeywordMatchType.BROAD })),
];

console.log("\n" + "═".repeat(70));
console.log(LIVE ? "▶  LIVE RUN" : "▶  DRY-RUN");
console.log("═".repeat(70));
console.log(`\n[1] Ad group: "YouTube"  ENABLED  cpc_bid=10000 micros`);
console.log(`[2] Positive keywords (EXACT): ${POSITIVE_KEYWORDS.length}`);
for (const k of POSITIVE_KEYWORDS) console.log(`    [EXACT] ${k}`);
console.log(`\n[3] Negatives: ${ALL_NEGATIVES.length} (${igNegatives.length} IG FR + ${YOUTUBE_SPECIFIC_NEGATIVES.length} YouTube-specific)`);
