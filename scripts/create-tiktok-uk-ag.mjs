/**
 * Creates the Tiktok ad group on campaign [UK] Fanovera (23844174192).
 *
 * Mirrors the lean UK structure (Instagram UK = EXACT positives, carved
 * negatives, English whitehat RSA, 5 sitelinks, no price asset) and adapts
 * the mature [FR] Tiktok ad group's keywords/negatives to English.
 *
 * IMPORTANT — whitehat: RSA copy contains no "buy"/quantities/"cheap" (the
 * fragile-account rule). Those words live only in the keywords (targeting),
 * which is allowed. RSA is created PAUSED for manual policy review.
 *
 * NOTE — negative/positive conflict guard: the UK positives are EXACT and
 * contain buy/cheap/followers. So we deliberately do NOT negate "buy",
 * "cheap", "followers", "buy followers", or "cheap followers" — a BROAD
 * negative on any of those would suppress our own keywords.
 *
 * Usage:
 *   node scripts/create-tiktok-uk-ag.mjs          # dry-run
 *   node scripts/create-tiktok-uk-ag.mjs --live   # execute
 */
...
// ── Negatives (BROAD) — English + competitor brands, adapted from [FR] Tiktok.
// Deliberately omits buy / cheap / followers / "buy followers" / "cheap followers"
// to avoid suppressing the EXACT positives above.
...
console.log(`\nCampaign:  [UK] Fanovera (${UK_CAMPAIGN_ID})`);
...
const agRes = await customer.adGroups.create([{
  name: "Tiktok",
  campaign: `customers/${CUSTOMER_ID}/campaigns/${UK_CAMPAIGN_ID}`,
...
console.log(`\n✅ DONE. AG ${adGroupId} "Tiktok" on [UK] Fanovera (RSA PAUSED — enable after policy review).`);
console.log(`URL: https://ads.google.com/aw/adgroups?campaignId=${UK_CAMPAIGN_ID}`);
