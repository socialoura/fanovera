      ...
      <div className="container" style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ textAlign: "center", maxWidth: 760, margin: "0 auto 24px" }}>
          ...
          <h1 className="display" style={{ margin: 0, fontSize: "clamp(26px, 4.4vw, 48px)" }}>
            Des followers Twitch <span className="squiggle tw">qualifiés</span> & ciblés.
          </h1>
        </div>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div className="pack-grid" style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 14, marginBottom: 32 }}>
            {PACKS.map((p, i) => (
              <button key={i} onClick={() => setPack(i)} className={`pack-tile tw ${pack === i ? "selected" : ""} ${p.popular ? "popular" : ""} ${p.best ? "best" : ""}`}>
        ...
        <div style={{ maxWidth: 480, margin: "0 auto" }}>
          <div style={{ background: "white", border: "2px solid var(--tw-purple)", borderRadius: 22, padding: 24, position: "relative", boxShadow: "0 18px 40px -16px rgba(145,70,255,0.35)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              <NetIcon kind="twitch" color="var(--tw-purple)" size={20} />
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--tw-purple)" }}>Votre commande</div>
            </div>
