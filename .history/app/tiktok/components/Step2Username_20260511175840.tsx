...
export default function Step2Username({ country, pack, username, setUsername, onNext, onBack }: Props) {
      ...
      <div className="container" style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ textAlign: "center", maxWidth: 720, margin: "0 auto 36px" }}>
          <h1 className="display" style={{ fontSize: "clamp(32px, 4vw, 52px)", margin: "0 0 12px" }}>
            À quel <span className="squiggle tt">compte</span> les envoyer ?
          </h1>
          <p style={{ maxWidth: 540, margin: "0 auto", fontSize: 16, color: "var(--ink-2)", lineHeight: 1.55 }}>
            Entrez juste votre <strong style={{ color: "var(--ink)" }}>pseudo TikTok</strong>, c&apos;est tout. Aucun mot de passe, aucun accès demandé. Le compte doit être <strong style={{ color: "var(--ink)" }}>public</strong>.
          </p>
        </div>

        <div className="checkout-grid" style={{ display: "grid", gridTemplateColumns: "1fr 0.9fr", gap: 28, maxWidth: 1000, margin: "0 auto" }}>
          <div style={{ background: "white", border: "1px solid var(--line)", borderRadius: 22, padding: 28 }}>
            ...
            {touched && !valid && clean.length > 0 && (
              <div style={{ marginTop: 10, fontSize: 13, color: "var(--tt-red)", display: "flex", gap: 6, alignItems: "center" }}>
                <span>⚠</span> Format invalide. Lettres, chiffres, &quot;.&quot; et &quot;_&quot; uniquement (2-30 caractères).
              </div>
            )}
            ...
            <div style={{ marginTop: 24, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[
                ...
                <div key={i} style={{ padding: "10px 12px", background: "var(--paper-2)", borderRadius: 12, fontSize: 13, fontWeight: 600, display: "flex", gap: 8, alignItems: "center" }}>
                  <span>{c.i}</span>
                  <span>{c.t}</span>
                </div>
              ))}
          ...
          <div style={{ position: "relative" }}>
            <div className="tt-card">
              <div style={{ display: "flex", alignItems: "center", gap: 12, padding: 16, borderBottom: "1px solid var(--line)" }}>
                <div style={{ width: 56, height: 56, borderRadius: "50%", background: "linear-gradient(135deg, var(--tt-cyan), var(--tt-red))", display: "grid", placeItems: "center", color: "white", fontWeight: 800, fontSize: 22 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: "var(--ink-3)", marginTop: 2 }}>
                    {verified ? "Compte trouvé · Public" : verifying ? "Recherche…" : "En attente du pseudo"}
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", textAlign: "center", padding: "16px 0", borderBottom: "1px solid var(--line)" }}>
                ...
                <div style={{ borderLeft: "1px solid var(--line)", borderRight: "1px solid var(--line)" }}>
                  <div style={{ fontWeight: 800, fontSize: 18, color: verified ? "var(--tt-red)" : "var(--ink)" }}>
                    {verified ? (
                      <span>
                        1 842 <span style={{ fontSize: 12, color: "var(--green)" }}>
                ...
                <div>
                  <div style={{ fontWeight: 800, fontSize: 18 }}>28,4K</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 4, padding: 4 }}>
                {[1,2,3,4,5,6].map((i) => (
                  <div key={i} style={{ aspectRatio: "9/16", background: `linear-gradient(135deg, var(--tt-cyan), var(--tt-red))`, opacity: 0.85, borderRadius: 4, display: "grid", placeItems: "center", color: "white" }}>
