    ...
    <section className="slide-in" style={{ padding: "40px 0 0", position: "relative" }}>
      ...
      <div className="container" style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ textAlign: "center", maxWidth: 760, margin: "0 auto 24px" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", background: "linear-gradient(135deg, rgba(37,244,238,0.12), rgba(254,44,85,0.12))", color: "var(--tt-red)", fontSize: 12, fontWeight: 800, letterSpacing: "0.08em", borderRadius: 999, marginBottom: 16, textTransform: "uppercase" }}>
            <NetIcon kind="tiktok" color="var(--tt-red)" size={14} />
            TikTok
          </div>
          <h1 className="display" style={{ margin: 0, fontSize: "clamp(26px, 4.4vw, 48px)" }}>
            Des followers TikTok <span className="squiggle tt">100% réels</span> & actifs.
          </h1>
        </div>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div className="pack-grid" style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 14, marginBottom: 32 }}>
            {PACKS.map((p, i) => (
              <button key={i} onClick={() => setPack(i)} className={`pack-tile ${pack === i ? "selected" : ""} ${p.popular ? "popular" : ""} ${p.best ? "best" : ""}`}>
                ...
                <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px dashed var(--line)" }}>
                  <div style={{ fontSize: 18, fontWeight: 800, color: pack === i ? "var(--tt-red)" : "var(--ink)", letterSpacing: "-0.01em" }}>
        ...
        <div className="checkout-grid" style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 24, maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ background: "white", border: "1px solid var(--line)", borderRadius: 22, padding: 24 }}>
            <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--ink-3)", marginBottom: 8 }}>
              Inclus dans votre pack
            </div>
            {[
              { i: "⚡", c: "rgba(82,96,230,0.12)", t: "Livraison en", strong: country === "fr" ? "47 secondes" : "37 secondes" },
              { i: "✓", c: "rgba(77,191,138,0.15)", t: "Profils", strong: country === "fr" ? "100% Français vérifiés" : "100% Européens vérifiés" },
              { i: "🛡", c: "rgba(245,184,0,0.15)", t: "", strong: "Garantie à vie en cas de chute" },
              { i: "🔄", c: "rgba(255,107,155,0.15)", t: "", strong: "100% satisfait ou remboursé sous 30j" },
              { i: "👤", c: "rgba(255,138,76,0.15)", t: "", strong: "Pas besoin de votre mot de passe" },
              { i: "💳", c: "rgba(82,96,230,0.12)", t: "", strong: "CB · Apple Pay · Google Pay · PayPal" },
              { i: "🇫🇷", c: "var(--paper-2)", t: "", strong: "Service client français 7j/7" },
            ].map((r, i) => (
              <div key={i} className="trust-row">
                <div className="trust-icon" style={{ background: r.c, fontSize: 14 }}>{r.i}</div>
                <div>
          ...
          <div style={{ background: "white", border: "2px solid var(--tt-red)", borderRadius: 22, padding: 24, position: "relative", boxShadow: "0 18px 40px -16px rgba(254,44,85,0.3)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              <NetIcon kind="tiktok" color="var(--tt-red)" size={20} />
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--tt-red)" }}>
                Votre commande
              </div>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", padding: "14px 0" }}>
              <div>
                ...
                <div style={{ fontSize: 32, fontWeight: 800, color: "var(--tt-red)", letterSpacing: "-0.02em", lineHeight: 1, marginTop: 4 }}>
