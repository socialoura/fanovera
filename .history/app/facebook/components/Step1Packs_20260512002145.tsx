    ...
    <section className="slide-in" style={{ padding: "40px 0 0", position: "relative" }}>
      ...
      <div className="container" style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ textAlign: "center", maxWidth: 760, margin: "0 auto 24px" }}>
          <div className="hide-md" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", background: "linear-gradient(135deg, rgba(24,119,242,0.10), rgba(22,111,229,0.10))", color: "var(--fb-blue)", fontSize: 12, fontWeight: 800, letterSpacing: "0.08em", borderRadius: 999, marginBottom: 16, textTransform: "uppercase" }}>
            <NetIcon kind="facebook" color="var(--fb-blue)" size={14} /> Facebook
          </div>
          <h1 className="display" style={{ margin: 0, fontSize: "clamp(26px, 4.4vw, 48px)" }}>
            Des likes Facebook <span className="squiggle fb">qualifiés</span> & ciblés.
          </h1>
        </div>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div className="pack-grid" style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 14, marginBottom: 32 }}>
            {PACKS.map((p, i) => (
              <button key={i} onClick={() => handlePackClick(i)} className={`pack-tile fb ${pack === i ? "selected" : ""} ${p.popular ? "popular" : ""} ${p.best ? "best" : ""}`}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "var(--ink-3)", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 4 }}>Likes</div>
                ...
                <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px dashed var(--line)" }}>
                  <div style={{ fontSize: 18, fontWeight: 800, color: pack === i ? "var(--fb-blue)" : "var(--ink)", letterSpacing: "-0.01em" }}>{formatPrice(p, country)}</div>
        ...
        <div style={{ maxWidth: 480, margin: "0 auto" }}>
          <div ref={orderCardRef} style={{ background: "white", border: "2px solid var(--fb-blue)", borderRadius: 22, padding: 24, position: "relative", boxShadow: "0 18px 40px -16px rgba(24,119,242,0.3)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              <NetIcon kind="facebook" color="var(--fb-blue)" size={20} />
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--fb-blue)" }}>Votre commande</div>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0", borderBottom: "1px dashed var(--line)" }}>
              <div>
                ...
                <div style={{ fontWeight: 700, fontSize: 16, marginTop: 2 }}>
                  {formatQty(selectedPack.qty)} likes
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", padding: "14px 0" }}>
              <div>
                ...
                <div style={{ fontSize: 32, fontWeight: 800, color: "var(--fb-blue)", letterSpacing: "-0.02em", lineHeight: 1, marginTop: 4 }}>{formatPrice(selectedPack, country)}</div>
              </div>
            ...
            <button className="btn-primary btn-fb" onClick={onNext} style={{ width: "100%", padding: "16px 26px", fontSize: 16, marginTop: 8 }}>
              Continuer
