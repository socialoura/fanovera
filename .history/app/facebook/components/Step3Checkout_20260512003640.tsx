...
import type { FbProfile } from "./Step2Page";
...
export default function Step3Checkout({ country, pack, pageInput, email, profile, clientSecret, onBack, onBackToPacks }: Props) {
    ...
    <section className="slide-in" style={{ padding: "40px 0 56px", position: "relative" }}>
      ...
      <div className="container" style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          <div className="checkout-recap fb" style={{ background: "white", borderRadius: 22, padding: 24 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
              <NetIcon kind="facebook" color="var(--fb-blue)" size={20} />
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--fb-blue)" }}>Récapitulatif</div>
            </div>

            <div style={{ padding: "14px", background: "linear-gradient(135deg, rgba(24,119,242,0.08), rgba(22,111,229,0.06))", borderRadius: 14, display: "flex", gap: 12, alignItems: "center", marginBottom: 18 }}>
              <div style={{ width: 48, height: 48, borderRadius: "50%", background: "linear-gradient(135deg, var(--fb-blue), var(--fb-blue-2))", display: "grid", placeItems: "center", color: "white", fontWeight: 800, fontSize: 20, overflow: "hidden", flexShrink: 0 }}>
                {profile?.avatarUrl ? (
                  <Image src={profile.avatarUrl} alt={profile.name} width={48} height={48} unoptimized style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                ) : (
                  <span>{profile?.handle ? profile.handle.charAt(0).toUpperCase() : "?"}</span>
                )}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 11, color: "var(--ink-3)", fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase" }}>Page destinataire</div>
                <div style={{ fontWeight: 700, fontSize: 15, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {profile?.name || "Votre page Facebook"}
                </div>
                {profile?.handle && (
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", fontSize: 14 }}>
              <div>
                <div style={{ fontWeight: 600 }}>{formatQty(PACKS[pack].qty)} likes Facebook</div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", fontSize: 14, color: "var(--green)", borderBottom: "1px dashed var(--line)" }}>
              <div>
                <div style={{ fontWeight: 600 }}>+{formatQty(PACKS[pack].bonus)} likes offerts</div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", padding: "16px 0 4px" }}>
              ...
              <div style={{ fontSize: 32, fontWeight: 800, color: "var(--fb-blue)", letterSpacing: "-0.02em", lineHeight: 1 }}>
                {fmtEuro(total)}
              </div>
            </div>
