...
export default function Step2Username({
  ...
  const handleNext = () => {
    if (!username.trim()) {
      setSubmitError("Entrez votre pseudo Instagram.");
      return;
    }
    if (!verified) {
      if (verifying) setSubmitError("Vérification du compte en cours…");
      else if (apiError === "not_found") setSubmitError("Ce compte est introuvable sur Instagram.");
      else if (apiError === "private") setSubmitError("Ce compte est privé. Passez-le en public pour continuer.");
      else setSubmitError("Entrez un pseudo Instagram valide et public.");
      return;
    }
    if (!emailValid) {
      ...
      <div className="container" style={{ position: "relative", zIndex: 1 }}>
        ...
        >
          ...
          <div className="ig-preview-col" style={{ position: "relative" }}>
            ...
            {/* Floating sticker */}
            <div
              className="floaty ig-new-total-sticker"
              style={
                {
                  position: "absolute",
                  top: -14,
