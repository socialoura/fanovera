...
export default function Step1Packs({ country, pack, setPack, onNext }: Props) {
  ...
  return (
    <section className="slide-in" style={{ padding: "40px 0 0", position: "relative" }}>
      <YtSprinkle count={6} seed={0} />
      <div className="container" style={{ position: "relative", zIndex: 1 }}>
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 12 }}>
          <div className="show-md-only" style={{ alignItems: "center", gap: 8, padding: "6px 14px", background: "linear-gradient(135deg, rgba(255,0,0,0.10), rgba(204,0,0,0.10))", color: "var(--yt-red)", fontSize: 12, fontWeight: 800, letterSpacing: "0.08em", borderRadius: 999, textTransform: "uppercase" }}>
            <NetIcon kind="youtube" color="var(--yt-red)" size={14} /> YouTube
          </div>
        </div>
        <Stepper step={1} />

        <div style={{ textAlign: "center", maxWidth: 760, margin: "0 auto 24px" }}>
          <div className="hide-md" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", background: "linear-gradient(135deg, rgba(255,0,0,0.10), rgba(204,0,0,0.10))", color: "var(--yt-red)", fontSize: 12, fontWeight: 800, letterSpacing: "0.08em", borderRadius: 999, marginBottom: 16, textTransform: "uppercase" }}>
            <NetIcon kind="youtube" color="var(--yt-red)" size={14} /> YouTube
          </div>
          <h1 className="display" style={{ margin: 0, fontSize: "clamp(26px, 4.4vw, 48px)" }}>
            Une visibilite YouTube <span className="squiggle yt">ciblee</span> & progressive.
          </h1>
        </div>
          ...
          <div className="pack-grid" style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 14, marginBottom: 32 }}>
            {PACKS.map((p, i) => (
              <button key={i} onClick={() => handlePackClick(i)} className={"pack-tile yt" + (pack === i ? " selected" : "") + (p.popular ? " popular" : "") + (p.best ? " best" : "")}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "var(--ink-3)", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 4 }}>Vues</div>
        ...
        <div style={{ maxWidth: 480, margin: "0 auto" }}>
          <div ref={orderCardRef} style={{ background: "white", border: "2px solid var(--yt-red)", borderRadius: 22, padding: 24, position: "relative", boxShadow: "0 18px 40px -16px rgba(255,0,0,0.3)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              <NetIcon kind="youtube" color="var(--yt-red)" size={20} />
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--yt-red)" }}>Votre campagne</div>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0", borderBottom: "1px dashed var(--line)" }}>
              <div>
                ...
                <div style={{ fontWeight: 700, fontSize: 16, marginTop: 2 }}>
                  Pack visibilite YouTube {formatQty(selectedPack.qty)}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", padding: "14px 0" }}>
              <div>
                ...
                <div style={{ fontSize: 32, fontWeight: 800, color: "var(--yt-red)", letterSpacing: "-0.02em", lineHeight: 1, marginTop: 4 }}>{formatPrice(selectedPack, country)}</div>
              </div>
            ...
            <button className="btn-primary btn-yt" onClick={onNext} style={{ width: "100%", padding: "16px 26px", fontSize: 16, marginTop: 8 }}>
              Continuer
