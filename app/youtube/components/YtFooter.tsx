        ...
        <div className="footer-grid" style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", gap: 40, marginBottom: 40, paddingBottom: 40, borderBottom: "1px solid var(--line)" }}>
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 20, flexWrap: "wrap" }}>
              {NETWORKS.map((n) => (
                <a key={n.id} href={"/" + n.id} aria-label={n.name} title={n.name} className={`net-icon ${n.id === "youtube" ? "active" : ""}`}>
                  <NetIcon kind={n.icon} color={n.id === "youtube" ? "white" : "currentColor"} size={18} />
                </a>
              ))}
            </div>
          </div>
