...
const QUERY = `
SELECT
  properties.tt2_packs_variant AS variant,
  count(DISTINCT if(event = 'tt2_packs_exposed', person_id, NULL)) AS exposed,
...
FROM events
WHERE properties.tt2_packs_variant IN ('chips', 'slider')
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error("tt2-packs results error:", error);
