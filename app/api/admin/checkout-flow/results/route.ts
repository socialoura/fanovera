...
// Per-variant funnel for the Instagram merged-checkout A/B, read from PostHog
// via the `checkout_flow_variant` super property carried on every event of an
// exposed visitor. `amount` is in cents; revenue mixes currencies (mostly GBP
// for the GB campaign) — fine for a directional per-arm comparison.
const QUERY = `
SELECT
  properties.checkout_flow_variant AS variant,
  count(DISTINCT if(event = 'checkout_flow_exposed', person_id, NULL)) AS exposed,
...
FROM events
WHERE properties.checkout_flow_variant IN ('control', 'merged')
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error("checkout-flow results error:", error);
