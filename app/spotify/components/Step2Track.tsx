...
export default function Step2Track({
  ...
  const validUrl = mode === "url" && SP_RE.test(url);
    ...
    const debounce = setTimeout(async () => {
      ...
      try {
        const qs = validUrl
          ...
          : `name=${encodeURIComponent(trackName.trim())}&artist=${encodeURIComponent(artistName.trim())}`;
        const res = await fetch(`/api/spotify/preview?${qs}`, { signal: controller.signal });
        const json = await res.json();
        if (controller.signal.aborted) return;
        if (!res.ok || !json.trackName) {
          setApiError("not_found");
          setVerified(false);
        } else {
          setProfile(json as SpoPreview);
          setVerified(true);
        }
      } catch (err) {
  ...
  }, [hasInput, validUrl, url, trackName, artistName, setProfile]);
