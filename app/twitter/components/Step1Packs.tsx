...
export default function Step1Packs({ country, pack, setPack, onNext, productType, setProductType }: Props) {
  ...
  const audienceLabel = productType === "likes" ? t.audienceLikes : productType === "retweets" ? t.audienceRetweets : t.audience;
  ...
  return (
    <section data-i18n-skip className="slide-in" style={{ padding: "40px 0 0", position: "relative" }}>
      <XSprinkle count={6} seed={0} />
      <div className="container" style={{ position: "relative", zIndex: 1 }}>
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 12 }}>
          <div className="show-md-only" style={{ alignItems: "center", gap: 8, padding: "6px 14px", background: "linear-gradient(135deg, rgba(0,0,0,0.08), rgba(29,155,240,0.10))", color: "var(--x-ink)", fontSize: 12, fontWeight: 800, letterSpacing: "0.08em", borderRadius: 999, textTransform: "uppercase" }}>
        ...
        <div style={{ textAlign: "center", maxWidth: 760, margin: "0 auto 24px" }}>
          ...
          <div className="x-mode-toggle" style={{ marginTop: 20, marginBottom: 0 }}>
            ...
            <button className={productType === "retweets" ? "active" : ""} onClick={() => switchProduct("retweets")}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div className="pack-grid" style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 14, marginBottom: 32 }}>
            {packs.map((p, i) => (
              <button key={i} onClick={() => handlePackClick(i)} className={"pack-tile x" + (safePack === i ? " selected" : "") + (p.popular ? " popular" : "") + (p.best ? " best" : "")}>
                ...
                <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px dashed var(--line)" }}>
                  <div style={{ fontSize: 18, fontWeight: 800, color: safePack === i ? "var(--x-ink)" : "var(--ink)", letterSpacing: "-0.01em" }}>{formatPrice(p, country)}</div>
        ...
        <div style={{ maxWidth: 480, margin: "0 auto" }}>
          <div ref={orderCardRef} style={{ background: "white", border: "2px solid var(--x-ink)", borderRadius: 22, padding: 24, position: "relative", boxShadow: "0 18px 40px -16px rgba(0,0,0,0.3)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--x-ink)" }}>{t.campaign}</div>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", padding: "14px 0" }}>
              <div>
                ...
                <div style={{ fontSize: 32, fontWeight: 800, color: "var(--x-ink)", letterSpacing: "-0.02em", lineHeight: 1, marginTop: 4 }}>{formatPrice(selectedPack, country)}</div>
              </div>
            ...
            <button className="btn-primary btn-x" onClick={onNext} style={{ width: "100%", padding: "16px 26px", fontSize: 16, marginTop: 8 }}>
              {t.continue}
