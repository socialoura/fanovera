...
import { formatQty, fmtEuro } from "../data";
...
export default function Step4Checkout({ country, profile, username, email, setEmail, sel, selectedPosts, needsPosts, onBack, onBackToStart }: Props) {
  ...
  const labels: Record<ProductKey, string> = {
    followers: ttCopy.step1.productFollowers,
    likes: ttCopy.step1.productLikes,
    views: ttCopy.step1.productViews,
  };
  ...
  // Build the multi-product cart. Likes/views carry the selected videos so
  // fulfillment spreads each pack across them (one BulkFollows sub-order/video).
      ...
      <div className="container" style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          <div style={{ background: "white", border: "2px solid var(--tt-red)", borderRadius: 22, padding: 24 }}>
            ...
            {/* Recipient */}
            <div style={{ padding: 14, background: "linear-gradient(135deg, rgba(37,244,238,0.08), rgba(254,44,85,0.08))", borderRadius: 14, display: "flex", gap: 12, alignItems: "center", marginBottom: 16 }}>
              ...
              return (
                <div key={k} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: "10px 0", fontSize: 14, borderBottom: "1px dashed var(--line)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, flexShrink: 0, background: meta.accent === "cyan" ? "var(--tt-cyan-soft)" : "var(--tt-red-soft)", display: "grid", placeItems: "center", color: meta.accent === "cyan" ? "#0a9b96" : "var(--tt-red)" }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      {meta.needsPosts && postUrls.length > 0 && (
                        <div style={{ fontSize: 11, color: "var(--ink-3)" }}>
                          {locale === "fr" ? `réparti sur ${postUrls.length} vidéo${postUrls.length > 1 ? "s" : ""}` : `across ${postUrls.length} video${postUrls.length > 1 ? "s" : ""}`}
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", padding: "16px 0 4px" }}>
              ...
              <div style={{ fontSize: 32, fontWeight: 800, color: "var(--tt-red)", letterSpacing: "-0.02em", lineHeight: 1 }}>{fmtEuro(total)}</div>
            </div>

            {/* Email */}
            <div style={{ borderTop: "1px dashed var(--line)", paddingTop: 18, marginTop: 14 }}>
              <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12, marginBottom: 10, flexWrap: "wrap" }}>
                <label htmlFor="tt2-email" style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--ink-3)" }}>{t2.emailLabel}</label>
                <span style={{ fontSize: 13, fontWeight: 700, color: "var(--ink-2)" }}>{t2.emailHint}</span>
              </div>
              <div className="input-shell tt">
                <input
                  id="tt2-email"
                  type="email"
                  inputMode="email"
