...
export default function LiSprinkle({ count = 6, seed = 0 }: { count?: number; seed?: number }) {
  ...
  return (
    <>
      {items.map((p, i) => (
        <div
          key={i}
          className={`li-deco ${p.anim} ${p.tint}`}
          style={{ top: p.top, left: p.left, right: p.right, ["--rot" as string]: p.rot + "deg" } as CSSProperties}
        >
          <NetIcon kind="linkedin" color="currentColor" size={p.size} />
        </div>
      ))}
    </>
  );
