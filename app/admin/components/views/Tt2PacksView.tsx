...
import type { Tt2PacksMode } from "@/app/lib/tt2PacksExperiment.server";

const OPTIONS: { mode: Tt2PacksMode; label: string; sub: string; color: string }[] = [
  { mode: "off", label: "Désactivé", sub: "Tout le monde voit les pastilles (contrôle).", color: "#94a3b8" },
  { mode: "ab", label: "A/B 50/50", sub: "Moitié pastilles, moitié slider. Test en cours.", color: "#f59e0b" },
  { mode: "force_slider", label: "Forcer slider", sub: "Tout le monde voit le slider (gagnant verrouillé).", color: "#22c55e" },
  { mode: "force_chips", label: "Forcer pastilles", sub: "Tout le monde voit les pastilles (gagnant verrouillé).", color: "#22c55e" },
];
...
export default function Tt2PacksView() {
  const [mode, setMode] = useState<Tt2PacksMode | null>(null);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState<Tt2PacksMode | null>(null);
  ...
  const loadResults = async () => {
    ...
    setResultsError("");
    try {
      const res = await fetch("/api/admin/tt2-packs/results", { headers: authHeaders() });
  ...
  useEffect(() => {
    fetch("/api/admin/tt2-packs", { headers: authHeaders() })
      .then(async (res) => {
        const data = (await res.json()) as { mode?: Tt2PacksMode; error?: string };
  ...
  const updateMode = async (next: Tt2PacksMode) => {
    if (mode === next) return;
    setSaving(next);
    setMessage("");
    setError("");
    try {
      const res = await fetch("/api/admin/tt2-packs", {
        method: "POST",
      ...
      setMode(next);
      setMessage(`A/B Packs → ${OPTIONS.find((o) => o.mode === next)?.label}`);
    } catch (err) {
      ...
      <section className="card" style={{ marginTop: 16, padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--a-line)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div>
            ...
            <div className="card-sub">KPI clé = <strong>revenu / visiteur</strong>. Source : PostHog (super-propriété <code>tt2_packs_variant</code>).</div>
          </div>
        ...
        {!resultsConfigured ? (
          ...
          (() => {
            ...
            const rows = ["chips", "slider"].map(byVariant);
            ...
            return (
              <>
                ...
                <div style={{ padding: "12px 20px", fontSize: 12, color: "var(--a-ink-3)", lineHeight: 1.6, borderTop: "1px solid var(--a-line)" }}>
                  {totalExposed === 0
                    ? "Aucune exposition encore — laisse tourner en A/B et attends du trafic."
                    : "🏆 = meilleur revenu/visiteur. "}
                  *Revenu en devises mêlées — comparaison directionnelle entre variantes, pas un total comptable. Attends plusieurs centaines d&apos;exposés/variante avant de conclure, puis verrouille avec <strong>Forcer slider</strong> / <strong>Forcer pastilles</strong>.
                </div>
              </>
            );
          })()
