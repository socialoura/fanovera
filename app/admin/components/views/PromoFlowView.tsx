...
export default function PromoFlowView() {
  ...
  const loadResults = async () => {
    ...
    setResultsError("");
    try {
      const res = await fetch("/api/admin/promo-flow/results", { headers: authHeaders() });
      ...
      <section className="card" style={{ marginTop: 16, padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--a-line)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div>
            ...
            <div className="card-sub">KPI clé = <strong>revenu / visiteur</strong>. Source : PostHog (super-propriété <code>promo_flow_variant</code>).</div>
          </div>
        ...
        {!resultsConfigured ? (
          ...
          (() => {
            ...
            const rows = ["control", "username_first"].map(byVariant);
            ...
            return (
              <>
                ...
                <div style={{ padding: "12px 20px", fontSize: 12, color: "var(--a-ink-3)", lineHeight: 1.6, borderTop: "1px solid var(--a-line)" }}>
                  ...
                  *Revenu en devises mêlées (surtout GBP pour la campagne GB) — comparaison directionnelle entre variantes, pas un total comptable. Volume GB faible → attends plusieurs centaines d&apos;exposés/variante avant de conclure. Verrouille ensuite avec <strong>Forcer username-first</strong> ou <strong>Désactivé</strong>.
                </div>
              </>
            );
          })()
