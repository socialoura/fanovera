...
export default function FbSprinkle({ count = 6, seed = 0 }: { count?: number; seed?: number }) {
  ...
  return (
    <>
      {items.map((p, i) => (
        <div
          key={i}
          className={`fb-deco ${p.anim} ${p.tint}`}
          style={{ top: p.top, left: p.left, right: p.right, ["--rot" as string]: p.rot + "deg" } as CSSProperties}
        >
          <NetIcon kind="facebook" color="currentColor" size={p.size} />
        </div>
      ))}
    </>
  );
